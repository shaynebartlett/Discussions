...
class DiscussionsModelCategory extends JModel { 
	...
	function getTotal() {
		...
		if ( empty( $this->_total)) {
			...
			$sql = "SELECT count(*) FROM " . $db->quoteName('#__discussions_messages') . " WHERE cat_id=" . $db->Quote($_catid) . " AND parent_id='0' AND published='1'";
	...
	function getCategoryMetaTitle() {
		...
		if ( empty( $this->_categoryMetaTitle)) {
            ...
            $categoryMetaTitleQuery = "SELECT meta_title FROM " . $db->quoteName( '#__discussions_categories') . " WHERE id=" . $db->Quote($_catid);
	...
	function getCategoryMetaDescription() {
		...
		if ( empty( $this->_categoryMetaDescription)) {
            ...
            $categoryMetaDescriptionQuery = "SELECT meta_description FROM " . $db->quoteName( '#__discussions_categories') . " WHERE id=" . $db->Quote($_catid);
	...
	function getCategoryMetaKeywords() {
		...
		if ( empty( $this->_categoryMetaKeywords)) {
            ...
            $categoryMetaKeywordsQuery = "SELECT meta_keywords FROM " . $db->quoteName( '#__discussions_categories') . " WHERE id=" . $db->Quote($_catid);
    ...
    /**
   	...
   	function getHtmlBoxTop() {
   		...
   		if ( empty( $this->_htmlBoxTop)) {
               ...
               $sql = "SELECT html_box_category_top FROM ".$db->quoteName( '#__discussions_configuration')." WHERE id='1'";
    ...
    /**
   	...
   	function getHtmlBoxBottom() {
   		...
   		if ( empty( $this->_htmlBoxBottom)) {
               ...
               $sql = "SELECT html_box_category_bottom FROM ".$db->quoteName( '#__discussions_configuration')." WHERE id='1'";
